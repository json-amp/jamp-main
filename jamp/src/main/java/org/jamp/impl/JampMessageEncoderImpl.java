    ...
    public String encode(JampMessage message) throws Exception {
       StringBuilder builder = new StringBuilder(255);
        ...
        if (message.getMessageType()==JampMessage.Type.SEND) {
            ...
            builder.append(String.format("[\"error\",\"%s\",\"%s\", %s]", message.getTo(), message.getFrom(), encoder.encode(message.getErrorObject())));            
        }  else if (message.getMessageType()==JampMessage.Type.ERROR_QUERY){
            builder.append(String.format("[\"error_query\",\"%s\",\"%s\",\"%s\", %s]", message.getQueryId(), message.getTo(), message.getFrom(), encoder.encode(message.getErrorObject())));            
        } else if (message.getMessageType()==JampMessage.Type.REPLY){
            builder.append(String.format("[\"reply\",\"%s\",\"%s\",\"%s\", %s]", message.getQueryId(), message.getTo(), message.getFrom(), encoder.encode(message.getReplyObject())));            
        }
