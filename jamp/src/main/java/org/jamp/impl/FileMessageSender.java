...
public class FileMessageSender implements JampMessageSender {
        ...
        if (!dir.isDirectory()) {
            if (!dir.mkdirs()) {
                throw new IllegalStateException(
                        "not able to create directory and it is not a directory "
                                + dir);
            }
        }
    ...
    public JampMessage sendMessage(JampMessage message) throws Exception {
        counter++;
        File outputFile = new File(dir, message.getToURL().getServiceName()
                + "." + message.getAction() + "_" + System.currentTimeMillis()
                + "_" + counter + ".jamp");
