...
public class SkeletonServiceInvokerImpl implements SkeletonServiceInvoker {
    ...
    public JampMessage invokeMessage(JampMessage message) throws Exception {
	    ...
	    List<Object> newArgs = converter.coerceFromListToFinalType(message.getArgs(), method.getParameterTypes());
    ...
    private Method findMethod(JampMessage message,
        ...
        if (method == null) {
            throw new IllegalStateException(Messages.getString("SkeletonServiceInvokerImpl.0")); //$NON-NLS-1$
        }
        return method;
    }
