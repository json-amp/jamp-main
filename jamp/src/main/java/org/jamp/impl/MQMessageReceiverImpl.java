...
public class MQMessageReceiverImpl implements MQMessageReciever {
    ...
    public MQMessageReceiverImpl(String connectionString, String login, String passcode, String destination, 
            Class<?> serviceClass, Object instance, JampMessageDecoder aMessageDecoder, JampMessageEncoder aMessageEncoder) throws IOException {
        connection = org.jamp.Factory.factory().createMQConnection(new JampMessageURL(connectionString));
        ...
        if (serviceClass!=null) {
            invoker = org.jamp.Factory.factory().createJampServerSkeleton(serviceClass);
        } else {
            invoker = org.jamp.Factory.factory().createJampServerSkeleton(instance);            
        }
    ...
    void handleTextMessage (String text) throws Exception{
        ...
        if (message.getMessageType()==JampMessage.Type.QUERY) {
            connection.send(message.getToURL().getServiceURI() + "_return", messageEncoder.encode(replyMessage));
        }
    }
